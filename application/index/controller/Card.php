...
use app\admin\library\Cashfree as Cashfreepay;
...
class Card extends Frontend
{
    ...
    public function _initialize(){
        ...
        if($order['channel_type'] != 'otc'){
            $this->_error('otc type exception');
        }
    ...
    public function index()
    {
        ...
        $minutes = (String)($this->valid_time($minutes));
        $seconds = (String)($this->valid_time($seconds));

        $timeArray = [
            'minutes' => substr($minutes, 0,1),
            'minutes2' => substr($minutes, 1,1),
            'seconds' => substr($seconds, 0,1),
        ...
        $upi = 'upi://pay?cu=INR&pa='.$this->otc_info['account_number'].'&pn=payment&tn='.$this->orderinfo['eshopno'].'&mc=0000&am='.$this->orderinfo['virtual_money'];
